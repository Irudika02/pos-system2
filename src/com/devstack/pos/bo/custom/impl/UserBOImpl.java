...
public class UserBOImpl implements UserBO {
   // private UserDao userDao= (UserDao) DaoFactory.getInstance().getDao(DaoType.USER);
    ...
    public ResponseUserDTO loginUser(String email, String password) throws SQLException, ClassNotFoundException {
        ...
        boolean passwordCheckStatus = PasswordHash.checkPassword(password, selectedUser.getPassword());

        if (passwordCheckStatus){
            return new ResponseUserDTO(selectedUser.getDisplay_name(),selectedUser.getContact_number(),200,true,"Login Successful");
        }else {
            return new ResponseUserDTO(null,null,401,false,"wrong Passworrd");
